...
[이미지 배치 지시 — 반드시 준수]
현재 상품 이미지가 1장 준비되어 있습니다. 본문 중 가장 어울리는 위치(보통 '들어가며' 직후) 딱 한 곳에만 [IMAGE_1] 자리표시자를 삽입하세요.
- 형식: [IMAGE_1] (대괄호 포함, 정확히 이 형식)
- [IMAGE_1]은 본문 전체에서 정확히 1번만 등장해야 합니다. 절대 두 번 이상 반복하지 마세요.
...
- 각 번호(예: [IMAGE_1])는 본문 전체에서 정확히 1번씩만 등장해야 합니다. 같은 번호를 절대 반복하지 마세요
- 자리표시자는 단독 줄에 배치하고 앞뒤로 빈 줄을 둘 것"""
